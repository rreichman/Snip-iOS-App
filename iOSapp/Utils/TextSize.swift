/**
 https://github.com/Instagram/IGListKit/issues/516
 **/
import Foundation
import UIKit
public struct TextSize {
    ...
    public static func sizeAttributed(_ text: NSAttributedString, font: UIFont, width: CGFloat, insets: UIEdgeInsets = UIEdgeInsets.zero) -> CGRect {
        
        let key = CacheEntry(text: text.string, font: font, width: width, insets: insets)
        if let hit = cache[key] {
            return hit
        }
        let constrainedSize = CGSize(width: width - insets.left - insets.right, height: CGFloat.greatestFiniteMagnitude)
        let options: NSStringDrawingOptions = [.usesFontLeading, .usesLineFragmentOrigin]
        let attributes = text.attributes(at: 0, longestEffectiveRange: nil, in: NSMakeRange(0, text.length))
        var bounds = (text.string as NSString).boundingRect(with: constrainedSize, options: options, attributes: attributes, context: nil)
        bounds.size.width = width
        bounds.size.height = ceil(bounds.height + insets.top + insets.bottom)
        cache[key] = bounds
        return bounds
